...
def main(gsheet_conn):
    ...
    st.write(
        "Welcome to the housing classification app. Please enter your name and upload your results file to check your accuracy and see the leaderboard."
    )
    ...
    if participant_name:
        ...
        if uploaded_file:
            participant_results = process_uploaded_file(
                uploaded_file, participant_name, gsheet_conn, RESULTS_PATH
            )
            display_participant_results(participant_results)
            update_and_plot_submissions(
                participant_results, participant_name, gsheet_conn
            )
        else:
