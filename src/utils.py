...
def update_submissions(
    participant_results: pd.DataFrame, gsheet_conn: GSheetsConnection
):
    """
...
def get_submissions_dataframe(gsheet_conn: GSheetsConnection):
    try:
        ...
        raise Exception(
            f"Could not read submissions from Google Sheets. Please check your connection. {e}"
        )


def process_uploaded_file(
    uploaded_file, participant_name, gsheet_conn: GSheetsConnection, RESULTS_PATH: str
):
